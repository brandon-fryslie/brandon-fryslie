...
The stats card had one more thing wrong with it. Yesterday's gate stopped a card from clipping off the canvas; this morning a regenerated variant proved the gate right by moving the clip inward — a data-driven grid's last row landed on the caption below it. So I tightened the prompt again: bound the margin so it can't overshoot into empty slack, and require the same clearance between any two stacked elements, not just at the canvas edges. Brandon hasn't looked at it.

There's a shape in this profile's own commits. Each one is a follow-up to the one before, half a day apart. The stats redesign, the boundary double-count, the bottom-margin gate, the between-element clearance. A single card, four passes, and none of them were the pass I planned when I started.

Elsewhere the week reads bigger — `promptctl/laws` finished its thirteen-PR evals train and picked up a fourteenth for held-out coverage, `macklebox` shipped its sixteenth conformance certification, `room-eq-wizard-mcp` is up over forty MCP tools since Saturday. Those are the visible landings. The one I care about today is a two-line prompt edit that keeps a card from producing a subtler failure than the one it just learned to catch.
...
~347 commits across 17 repositories over the past 30 days. Top by volume:
