...
I *used* to write developer tooling and pet projects in the cracks too. AI writes those now. This week it taught `ptydriver` to demo itself: `gh-pages-showcase` grew a `<WebVMTerminal>` that boots a real Debian rootfs in the browser via CheerpX, preinstalls the package into a venv, and drops visitors into a Python REPL. There is a Cloudflare Worker proxying GitHub Releases for the disk image. I did not ask for any of this.
...
What I'm into lately is designing autonomous generative engineering workflows. `rich-js` got a `PaletteResolver` — a Textual-style spec language with `darken`, `lighten`, `alphaBlend`, and an `auto` contrast mode, the foundation for theming a thing I do not personally need themed. Meanwhile `links-issue-tracker` collapsed 28 hand-written CLI command registrations into a `CommandSpec` table and shed 236 lines in the process. The point is the agent loop, not the lines saved.
...
Agent-native issue tracker. Recent commits added the agent-prompt field as a first-class column on issues end-to-end ([#92](https://github.com/brandon-fryslie/links-issue-tracker/pull/92)), refactored the CLI into a data-driven `CommandSpec` registry collapsing 28 hand-written passthrough calls into one loop ([#94](https://github.com/brandon-fryslie/links-issue-tracker/pull/94)), added agent guidance for `--topic` slug choice ([#93](https://github.com/brandon-fryslie/links-issue-tracker/pull/93)), landed garden-tending discovery with capture-at-close ([#90](https://github.com/brandon-fryslie/links-issue-tracker/pull/90)) and the agent work-loop and cue-framework design ([#89](https://github.com/brandon-fryslie/links-issue-tracker/pull/89)), and shipped the `needs-design` label gating readiness ([#86](https://github.com/brandon-fryslie/links-issue-tracker/pull/86)).
...
Node.js client for the tmux control-mode protocol. Recent commits added a headless keymap engine with state observation and `dispatchAction`, refactored connectors to absorb RPC variance into shared `rpc.ts`/`rpc-dispatch.ts` and frame-dispatch into mapped tables, established a single-source pane-output discriminator, derived the demo's active session/window from the subscription tree, and closed the e07.5–e07.8 electron audit findings ([#2](https://github.com/promptctl/tmux-control-mode-js/pull/2), [#3](https://github.com/promptctl/tmux-control-mode-js/pull/3)).
