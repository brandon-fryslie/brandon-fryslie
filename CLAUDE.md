...
- **The matrix jobs run concurrently because their write sets are disjoint, not because a limit permits it.** Each stages exactly the two files of its own slot, named on `archive`'s stdout — never globbed back out of the tree, so the stamp→path convention keeps one spelling. `STATS.md` is regenerated from the *whole* tree, which is why it is the one file all of them would have fought over, and why it belongs to the separate `gallery` job that runs once after the last card lands (checked out from `master`, not the dispatch SHA, or it would render the tree as it stood before the run). `max-parallel` is a budget for concurrent agent work, not a correctness guard — but restoring a directory-wide `git add` to a matrix job would make it one, and the pushes would start conflicting for real.
