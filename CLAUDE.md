...
|-------------|---------------------------------------------------------------------------|------------------|
| `doodle`    | `README.md` (DAILY-DOODLE block only), `DOODLES.md` (DOODLE-GALLERY block), `assets/daily-highlight.svg`, `assets/daily-stats.svg`, `assets/daily-stats.json`, `doodle-archive/` | Two freshly-authored creative outputs each day: the animated SVG "doodle", and the "Live GitHub Stats" card (`daily-stats.svg`), which Claude authors anew each day from the `daily-stats.json` data seam a prior mechanical step emits. Plus an archive copy of yesterday's doodle SVG and a prepended `DOODLES.md` gallery entry. The `<img>` tag inside `DAILY-DOODLE` is rewritten idempotently each run. |
...
- **`.github/scripts/generate-stats-svg.py`** — the stats *data*, not its final look. Picks 4 metrics from a pool of 8 with varying periods and writes `assets/daily-stats.json` (the data seam — the exact values, labels, and periods the card must render) plus a deterministic fallback SVG (to `/tmp` in CI). Enumeration metrics (days active, active repos, languages) come from the GraphQL `contributionsCollection` — exact and uncapped — *not* the 1000-result-capped commit search; pure counts use search `total_count`. Requires `GITHUB_TOKEN` and `pip install requests`. The doodle job runs it before Claude; Claude then authors the real `daily-stats.svg` from the seam (workflow **Task 3.5** — that prompt is where the card's creative direction lives). A later `python .github/scripts/generate-stats-svg.py --verify-svg assets/daily-stats.svg` step fails the run if any value isn't rendered, and Task 3.5 falls back to the deterministic card rather than ship a wrong or illegible one.
